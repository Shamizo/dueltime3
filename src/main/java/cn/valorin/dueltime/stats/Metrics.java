package cn.valorin.dueltime.stats;
...
public class Metrics {
    ...
    public Metrics(Plugin plugin, int pluginId) {
        ...
        if (enabled) {
            ...
            // Add custom charts of DuelTime
            DuelTimePlugin ins = DuelTimePlugin.getInstance();
            addCustomChart(new SingleLineChart("arenas", () -> ins.getArenaManager().size()));
            addCustomChart(new SingleLineChart("rewards_on_shop", () -> ins.getCacheManager().getShopCache().getList().size()));
            addCustomChart(new SingleLineChart("games_number", () -> gameNumber));
        }
    }

    private int gameNumber = 0;

    public void accumulateGameNumber() {
        gameNumber++;
    }
    ...
    private void startSubmitting() {
        final Runnable submitTask = () -> {
            ...
            SchedulerUtil.runTask(this::submitData);
        };
    ...
    private void submitData() {
        ...
        new Thread(() -> {
            try {
                ...
                // Reset the game number data of DuelTime regardless of whether it is sent successfully or not
                gameNumber = 0;
            }
        }).start();
    }
